...
set @Id_Rol = (SELECT top 1 R.RoleId FROM [dbo].[AspNetUsers] U inner join [dbo].[AspNetUserRoles] R on U.Id = R.UserId where U.UserName = @Usuario)
	...
	SELECT distinct	R.COD_REQUISICION,R.COD_ESTADO_REQUISICION,R.USUARIO_CREACION, R.EMAIL_USUARIO_CREACION, R.COD_CARGO, R.NOMBRE_CARGO, 
		...
		inner join [REQUISICION].[ROL_REQUISICION] RR on RR.ID_ROL = @id_Rol
		...
		Where R.USUARIO_CREACION = @Usuario AND R.COD_ESTADO_REQUISICION LIKE ISNULL(@COD_ESTADO_REQUISICION,R.COD_ESTADO_REQUISICION)
...
END
GO
/****** Object:  StoredProcedure [REQUISICION].[APROBAR_REQUISICION]    Script Date: 05/20/2019 10:36:32 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
-- =============================================
CREATE PROCEDURE [REQUISICION].[APROBAR_REQUISICION] 
	@ROL_APROBADOR int,
	@COD_REQUISICION int,
	@USUARIO varchar(50)
AS
...
BEGIN
	...
	BEGIN TRY
		...
		IF @ROL_APROBADOR = (SELECT COD_ROL R FROM [REQUISICION].[ROL] R WHERE R.NOMBRE = 'Controller')
			BEGIN 
				SET @SIGUIENTE_ESTADO = 2 --APROBADO CONTROLLER
			END

		ELSE IF @ROL_APROBADOR = (SELECT COD_ROL R FROM [REQUISICION].[ROL] R WHERE R.NOMBRE = 'RRHH')
			BEGIN
				SET @SIGUIENTE_ESTADO = 5 --APROBADO RRHH
			END

		ELSE IF @ROL_APROBADOR = (SELECT COD_ROL R FROM [REQUISICION].[ROL] R WHERE R.NOMBRE = 'USC')
			BEGIN
				SET @SIGUIENTE_ESTADO = 7 --APROBADO USC
			END			
	
		ELSE IF @ROL_APROBADOR = (SELECT COD_ROL R FROM [REQUISICION].[ROL] R WHERE R.NOMBRE = 'BP')
			BEGIN
